...
Last month Scottish Government released the [Reconviction Rates in Scotland statistical bulletin for the 2016/17 cohort](https://www.gov.scot/publications/reconviction-rates-scotland-2016-17-offender-cohort/). One of the headline figures in this bulletin is the average number of reconvictions per offender (what we'll call the _reconviction rate_[^00]), and last month's bulletin shows that Scotland's reconviction rate has fallen steadily over the last fifteen years[^0]. Whilst this overall fall in the reconviction rate is undoubtedly good news, in this post we'll show that interpreting this trend is not as straightforward as it might first seem.

[^00]: Until 2017 Scottish Government also referred to the average number of reconvictions per offender as the reconviction rate, but as of 2018 Scottish Government now calls the percentage of offenders who are reconvicted the 'reconviction rate'. 
...
Second, as well as a fall in reconviction rates we've also seen substantial changes in number of men and women of different ages convicted (known as the _prevalence_ of conviction)[^1], with very pronounced falls in convictions for young people, and young men in particular, that are not seen in other groups. 
...
changes in the offender mix can affect the overall
...
reductions in reoffending and how much is due to changes in the mix of people convicted[^3]. Specifically, we use the Prithwith Das Gupta's specification of two techniques known as _standardization_ and _decomposition_[^7].
...
the standardized rates in Figure Two. 
The red line shows what the reconviction rate would have been if only the offender mix had changed, and the blue line what
the reconviction rate if only the average number of reconvictions per offender had changed.
The changing offender mix has played a role in reducing
...
the changing age-mix has had in reducing the conviction rate: `r round(dgtable$percent_dist[[1]])`% of the change in the average number of reconvictions per offender between 2004 and 2016 is due to the changing offender mix. Standardization and decomposition show us that Scotland's
...
Our results show this description doesn't quite capture the whole story, as the reconviction rate capture not only changes in the frequency with which offenders are reconvicted, but also changes in the demographic mix of people who are entering the justice system each year.
As well as measuring "how well we are managing and supporting those that enter our criminal justice system", the reconviction indicator is reflecting falls in youth conviction, perhaps due to changes to youth justice policy in Scotland which have [emphasised diversion from prosecution](http://scottishjusticematters.com/wp-content/uploads/Shrinking-YJ-population-SJM_5-1_April2017-18.pdf), or broader social changes which may have led to falling youth conviction[^8].
...
This short post has illustrated how tricky it can be to measure outcomes in a complicated and interconnected area like criminal justice. To interpret change in the overall reconvictions rate we need to take
