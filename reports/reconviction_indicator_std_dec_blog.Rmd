...
rdata %>%
  ...
  left_join(., rdata %>%
              ...
              summarise_at(vars(contains("num")), sum) %>%
              mutate(
                overall_rate = (num_reconvicted / num_offenders)*100
              ) %>% select(year,overall_rate)
            ) %>% 
  ggplot(., aes(x = year,y=rate, group = interaction(Age,Gender), col = Gender)) +
  scale_colour_manual(values=c("#FAAB18","#1380A1"))+
  geom_line(aes(y=overall_rate),col="grey70",lwd=.7)+
  geom_line() + 
  gghighlight(rate[year==2016]>rate[year==2004])+
  ylab("Reconviction Rate")+
  ggtitle("Figure 1")+
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_x_continuous(breaks=2004:2016,labels=2004:2016,limits=c(2004,2018))+
  geom_label(aes(x=2016,y=30.5,label="Men, 31-40"),col="#1380A1",hjust=0,label.size=NA)+
  geom_label(aes(x=2016,y=27.5,label="Women, 31-40"),col="#FAAB18",hjust=0,label.size=NA)+
  geom_label(aes(x=2014,y=29,label="National Average"),fontface = "bold",col="grey70",label.size=NA)+
  geom_label(aes(x=2016,y=18,label="Women, 40+"),col="#FAAB18",hjust=0,label.size=NA)+
  geom_label(aes(x=2016,y=20.5,label="Men, 40+"),col="#1380A1",hjust=0,label.size=NA)+
  NULL
  #gghighlight doesn't seem to want to give labels. it does when you do e.g., rate>30 but not when indexing e.g., rate[year==2004]>30. 
  #also can't get the legend back! ah well. manual labels it is..
...
For example, if women over 40, who have a low average reconviction rate, made up a larger proportion of all people convicted then the overall conviction rate would go down, even if the reconviction rate itself did not change for any age group.
...
dgtable %>% mutate(
  ...
  ) %>% rename(`Percentage of crude` = percentage_of_crude) %>%
  knitr::kable(.,digits=2) %>%
  kableExtra::row_spec(3, bold = T, color = "white", background = "#555555") %>%
...
[^8a]: Prior to 2019, another measure - the _average number of reconvictions per offender_ - was one of Scottish Governments' National Indicators, and claimed to be ["one of the main ways of measuring how well we are managing and supporting those that enter our criminal justice system."](https://www2.gov.scot/About/Performance/scotPerforms/indicator/reconviction). In just the same way as the reconviction rate discussed in this post, the average number of reconvictions per offender is influenced by fluctuations in the demographic mix of people entering the criminal justice system each year. It can be further decomposed into changes in the prevalence and _frequency_ of reconvictions as well as the offender mix.
