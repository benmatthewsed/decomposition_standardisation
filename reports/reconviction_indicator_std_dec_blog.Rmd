...
The headline figure in this bulletin is the average number of reconvictions per offender (indeed, prior to 2019 this measure was a National Indicator). 
These measure shows a steady long-term fall in the reconviction rate in Scotland[^0].  
...
Whilst this is overall fall in the reconviction rate is undoubtedly good news,
...
There is a sort of technical work-around for this issue:
we can use methods developed by demographers to apportion the amount of change in the reconviction rate that is due to actual
reductions in reoffending, and how much is due to changes in the mix of people convicted[^3]. Specifically, we use the [Prithwith Das Gupta](link)'s specification of two techniques known as _standardization_ and _decomposition_.
...
the changing age-mix has had in reducing the conviction rate: `r round(dgtable$percent_dist[[1]])`% of the change in the _average number of reconvictions per offender_ is attributable to the changing age/sex structure of the convicted population.
...
These results show this description doesn't quite capture the whole story,
and illustrate how tricky it can be to measure outcomes in a complicated and interconnected area like criminal justice. Because the reconviction indicator also
...
It may also be picking up on [broader social changes](http://journals.sagepub.com/doi/abs/10.1177/1477370817731706) which have led to this shift in the demographic profile of the
convicted population as a result of falling youth conviction.

To interpret change in the overall reconvictions rate, we need to take
