...
cdata<-read_xlsx(here("regional_crimetrends.xlsx"),sheet = 12,col_names=F)
...
cdata %>% mutate(
  ...
  group_by(laa) %>% summarise_at(vars(starts_with("HBBRG"),starts_with("MYP")),~sum(as.numeric(.))) -> cdata_new
  
cdata_new %>% select(laa,starts_with("HB")) %>%
  gather(year,vcrime,starts_with("HB")) %>%
  mutate(
...
violent
burglary<-burglary %>% rename("burglary"="vcrime")

saveRDS(left_join(violent,burglary),"violence_burglary_LAA.RDS")
